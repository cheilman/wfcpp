...
Image::Image(const Dimensions2D size, const int channels)
...
Image::Image(const int width, const int height, const int channels) : Image(Dimensions2D(width, height), channels) {}

Image::Image(PixelComponentType *data, const int width, const int height, const int channels)
...
std::unique_ptr<Image> Image::cropy(const GridPosition ul_corner, const GridPosition br_corner) const {
...
void Image::paste(const Image &src, GridPosition dest_ul_corner, bool with_alpha) {
    const int src_width = std::min(src.width(),
                                   this->width() - dest_ul_corner.x());  // TODO: Need a +1/-1 here?
    const int src_height = std::min(src.height(),
                                    this->height() - dest_ul_corner.y());  // TODO: Need a +1/-1 here?
