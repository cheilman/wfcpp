...
public:
  ...
  std::unique_ptr<Image> cropy(const GridPosition ul_corner,
                               const Dimensions2D dims) const {
    return this->cropy(ul_corner, GridPosition(ul_corner.x() + dims.width() - 1, ul_corner.y() + dims.height() - 1));
  }

  void paste(const Image &src, GridPosition dest_ul_corner);
...
protected:
  ...
  void write_line(GridPosition dest_pos, const Image &source, GridPosition src_pos, int line_width);
